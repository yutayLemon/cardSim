...
function twoCard(scene,width,height,p){
    ...
    newCard1.place(p.clone().add(new THREE.Vector3(0,height,gap)),newCard1.globalVertex.topLeft[1]);
    
    
    let newCard2 = new cardObj(scene,cardWidth);
    applyRotation(theta,0,0,newCard2.rotationMatrx);
    newCard2.updateGlobalPos();
    newCard2.place(p.clone().add(new THREE.Vector3(0,height,-gap)),newCard2.globalVertex.topLeft[0]);
